...
- Format gaps that are in-scope to know, not accidental omissions: Alacritty YAML (pre-0.13) is not read; built-in / compiled-in emulator presets in WezTerm (Rust binary / Lua), iTerm2 (binary plist / Cocoa defaults), and Windows Terminal (package defaults.json) do not exist as static user theme files on disk and are not scanned; iTerm2 entries tagged Calibrated rather than sRGB are treated as sRGB; 256-color slots beyond the base 16 are ignored; Ghostty dark/light pairs are listed separately in the picker, and Mirror writes them as paired auto-detect blocks under the preferred dark and light workbench themes without turning `window.autoDetectColorScheme` on.
